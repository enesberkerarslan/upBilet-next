...
import { PROFILE_DEFAULT_SECTION, profileSectionPath } from "@/lib/profile-path";
      ...
      <div className="container mx-auto flex max-w-[1280px] justify-between px-4 md:px-6">
        <div className="flex w-full items-center justify-between md:hidden">
          ...
          {hydrated ? (
            !isLoggedIn ? (
              <Link
                href={href("/giris")}
                className="rounded-full bg-[#615FFF] px-4 py-2 text-sm font-medium text-white"
              >
                {t("header.login")}
              </Link>
            ) : (
        ...
        <div className="hidden w-full items-center justify-between md:flex">
          ...
          <div className="flex items-center gap-3">
            <Suspense fallback={<span className="h-9 w-[120px] rounded-xl bg-gray-100" aria-hidden />}>
              <LanguageSwitcher />
            </Suspense>
            {hydrated ? (
              <>
                ...
                {!isLoggedIn ? (
                  <>
                    <Link
                      href={href("/giris")}
                      className="rounded-[32px] border border-[#E4E4E7] bg-transparent px-8 py-3 text-sm font-semibold text-[#18181B] hover:opacity-80"
                    >
                      {t("header.login")}
                    </Link>
                    <Link
          ...
          <div className="absolute left-0 top-0 h-full w-80 overflow-y-auto bg-white shadow-xl">
            <div className="p-6">
              <div className="mb-6 flex items-center justify-between gap-2">
                <Image src="/img/logo.svg" alt="UpBilet" width={120} height={32} className="h-8 w-auto" />
                <Suspense fallback={<span className="h-9 w-[100px] rounded-xl bg-gray-100" aria-hidden />}>
                  <LanguageSwitcher />
                </Suspense>
                ...
                {!isLoggedIn ? (
                  <>
                    <Link
                      href={href("/giris")}
                      onClick={() => setShowMobileMenu(false)}
                      className="block w-full rounded-2xl border border-gray-300 py-3 text-center font-medium text-gray-700"
                    >
                      {t("header.login")}
                    </Link>
                    <Link
