...
class SaleService {
  ...
  /**
   * Gerçek ödeme / Stripe olmadan satış kaydı (admin panel test akışı).
   * Tutarlar: liste fiyatı × adet + etkinlik müşteri komisyonu (%)+KDV; satıcı tarafı ilandaki sellerAmount × adet.
   */
  async createTestSale({ listingId, buyerMemberId, quantity = 1 }) {
    const qty = Math.max(1, parseInt(String(quantity), 10) || 1);
    if (!mongoose.Types.ObjectId.isValid(listingId)) {
      throw new Error('Geçersiz ilan kimliği');
    }
    if (!mongoose.Types.ObjectId.isValid(buyerMemberId)) {
      throw new Error('Geçersiz alıcı kimliği');
    }

    const listing = await Listing.findById(listingId).populate('eventId');
    if (!listing) throw new Error('İlan bulunamadı');
    if (listing.status !== 'active') {
      throw new Error('Sadece aktif ilanlar için test satışı oluşturulabilir');
    }

    const available = (listing.quantity || 0) - (listing.soldQuantity || 0);
    if (available < qty) {
      throw new Error(`Yetersiz kontenjan (kalan: ${available})`);
    }

    const buyer = await Member.findById(buyerMemberId);
    if (!buyer) throw new Error('Alıcı üye bulunamadı');

    const sellerId = listing.memberId.toString();
    if (sellerId === buyerMemberId.toString()) {
      throw new Error('Satıcı kendi ilanı için test alıcı olamaz; başka üye seçin');
    }

    const event = listing.eventId;
    if (!event || !event._id) throw new Error('Etkinlik bilgisi eksik');

    const round2 = (n) => Math.round(Number(n) * 100) / 100;
    const customerPct = Math.min(100, Math.max(0, Number(event.comissionCustomer) || 20));
    const listUnit = Number(listing.price);
    const sellerUnit = Number(listing.sellerAmount);
    if (!Number.isFinite(listUnit) || listUnit < 0) throw new Error('İlan liste fiyatı geçersiz');
    if (!Number.isFinite(sellerUnit) || sellerUnit < 0) throw new Error('İlan satıcı tutarı geçersiz');

    const buyerBase = round2(listUnit * qty);
    const serviceFee = round2(buyerBase * (customerPct / 100));
    const serviceFeeKdv = round2(serviceFee * 0.2);
    const totalAmount = round2(buyerBase + serviceFee + serviceFeeKdv);
    const sellerTotalAmount = round2(sellerUnit * qty);

    const ticketHolders = Array.from({ length: qty }, () => ({
    ...
    const sale = new Sale({
      ...
      serviceFee,
      serviceFeeKdv,
      listingPrice: listUnit,
      sellerAmount: sellerUnit,
      sellerTotalAmount,
      ...
      transactionId: `TEST-${Date.now()}-${new mongoose.Types.ObjectId().toString().slice(-8)}`,
      billingInfo: {
        city: '—',
        district: '—',
        address: 'Test satışı (admin paneli — gerçek ödeme yok)',
      },
    ...
    await sale.save();
    listing.soldQuantity = (listing.soldQuantity || 0) + qty;
    await listing.save();

    return Sale.findById(sale._id)
      .populate('eventId', 'name date location')
      .populate('listingId', 'category price quantity soldQuantity')
      .populate('seller', 'name surname email')
      .populate('buyer', 'name surname email');
  }
}
